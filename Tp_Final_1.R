...
g <- ggplot(estadisticos_df, aes(x = Numero, y = Media, group=Nombre)) +
  geom_line(aes(color = Nombre), size = 0.5, alpha = 1.5) +
  # geom_smooth(method = "lm", se = FALSE, aes(color = Nombre)) +
  facet_wrap(.~Nombre,nrow = 5) +
  labs(title = "Media y desvio de pp") +scale_x_continuous(name="Fecha",labels = estadisticos_df$Fecha[seq(1,366,by=50)],breaks = estadisticos_df$Numero[seq(1,366,by=50)])

g<- g+geom_ribbon(estadisticos_df,mapping=aes(ymin=Desv_rest,ymax=Desv_sum),fill="pink",alpha=0.5)
#g<- g+geom_line(estadisticos_df,mapping=aes(x = Numero, y = Desv_sum, group=Nombre),color="magenta")+ ##lo dejo plor las dudas
#geom_line(estadisticos_df,mapping=aes(x = Numero, y = Desv_rest, group=Nombre),color="magenta")

############# porcentaje de años lluviosos de todos los 4 de febrero para cada punto de grilla #############
...
#Mapa 
mapa<-ggplot(data=febrero_porcentaje_df) +
         geom_raster(aes(x=Longitudes, y=Latitudes,fill=Porcentaje)) +
         labs(title = "Porcentaje") 
